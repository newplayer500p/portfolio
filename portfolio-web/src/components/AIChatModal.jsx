...
export default function AIChatModal({ targetProject, profile, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md">
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-cyan-500/30 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[650px] max-h-[90vh]">
        ...
        <div className="px-6 py-2.5 bg-slate-950/60 border-b border-slate-800/80 flex items-center gap-2 overflow-x-auto">
          ...
          {suggestedQuestions.map((q, idx) => (
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-slate-800/80 hover:bg-cyan-950 hover:text-cyan-300 border border-slate-700/60 text-xs text-slate-300 whitespace-nowrap transition-all cursor-pointer"
            >
              {q}
            </button>
          ))}
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-950/40">
          ...
          {isTyping && (
            <div className="flex items-center gap-2 text-cyan-400 text-xs font-semibold pl-11">
        ...
        <div className="p-4 bg-slate-950 border-t border-slate-800">
          ...
          >
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl bg-slate-900 border border-slate-800 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-cyan-500"
            />
            <button
              type="submit"
              disabled={!input.trim()}
